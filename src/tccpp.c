...
include_done:
    ...
    case TOK_PRAGMA:
        ...
        break;
    case TOK_BINARY:
        /* #binary directive for UPP - specifies executable output path */
        /* Skip the 'exe' keyword if present */
        ch = file->buf_ptr[0];
        skip_spaces();
        
        /* Check if next token is 'exe' */
        if (ch == 'e') {
            char *start = file->buf_ptr;
            if (start[0] == 'e' && start[1] == 'x' && start[2] == 'e' && 
        ...
        /* Store the binary path in TCCState */
        if (s1->binary_path)
